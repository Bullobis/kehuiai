...
class ProfessionalInferenceEngine(private val context: Context) {
    ...
    object Workloads {
        val PRESETS = listOf(
            WorkflowPreset("realistic_photo", "真实照片", "📷", "逼真的摄影作品",
                ...
                listOf("照片级真实感", "自然光线"),
                { copy(guidanceScale = 7.5f, steps = 25, scheduler = comkuaihuiai.data.model.SchedulerType.DPMSOLVER_PLUS_PLUS_2M_KARRAS) }
            ),
            WorkflowPreset("anime_art", "动漫风格", "🎨", "精美动漫插画",
                BaseModelType.SD_1_5, SchedType.EULER_A, 30, 8.0f,
                listOf(Pair(512, 768), Pair(768, 1024)),
                listOf("动漫线条", "鲜艳色彩"),
                { copy(guidanceScale = 8.0f, steps = 30, scheduler = comkuaihuiai.data.model.SchedulerType.EULER_ANCESTRAL) }
            ),
            WorkflowPreset("fast_preview", "快速预览", "⚡", "快速生成草图",
                BaseModelType.SD_1_5, SchedType.LCM, 6, 1.0f,
                listOf(Pair(512, 512)),
                listOf("极速出图", "即时反馈"),
                { copy(guidanceScale = 1.0f, steps = 6, scheduler = comkuaihuiai.data.model.SchedulerType.LCM) }
            ),
    ...
    private fun validateParams(params: GenerationParams, mode: GenMode): GenerationParams {
        ...
        val validatedMode = when (mode) {
            GenMode.TXT2IMG -> comkuaihuiai.data.model.GenerationMode.TXT2IMG
            GenMode.IMG2IMG -> comkuaihuiai.data.model.GenerationMode.IMG2IMG
            GenMode.INPAINT -> comkuaihuiai.data.model.GenerationMode.INPAINT
            GenMode.OUTPAINT -> comkuaihuiai.data.model.GenerationMode.TXT2IMG
            GenMode.PIXELART -> comkuaihuiai.data.model.GenerationMode.TXT2IMG
            GenMode.UPSCALE -> comkuaihuiai.data.model.GenerationMode.TXT2IMG
        }
